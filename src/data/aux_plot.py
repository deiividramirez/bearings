...
Velocidad final -> {vx[-1], vy[-1], vz[-1], vyaw[-1]}
        ...
        if dron == 2 or dron == 1:
            ...
            ax[0][dron - 1].plot(time[NUM:], err_pix_t[NUM:], "r", label="|Error (px)|")
        ax[0][dron - 1].plot(
            [time[NUM], time[-1]],
            [err[-1], err[-1]],
            "k--",
...
if lider == "1":
    ...
    fig, ax = plt.subplots(
        ...
        1,
        figsize=(5, 10),
        num=f"Velocidades y errores para el dron {dron}",
        sharex=True,
    )
    ...
    for i in ["kp", "kv", "kd"]:
        try:
            lamb = np.loadtxt(f"{path}/out/out_lambda_{i}_{dron}.txt")
            if np.any(lamb != 0):
                ax[2].plot(time[NUM:], lamb[NUM:], label="$\lambda_{" + i + "}$")
                ax[2].plot(
                    [time[NUM], time[-1]],
                    [lamb[-1], lamb[-1]],
                    "k--",
    ...
    plt.tight_layout()
    plt.savefig(
        f"{path}/out_velocidades_{dron}.png", bbox_inches="tight", pad_inches=0.1
    )
    ...
    plt.tight_layout()
    plt.savefig(f"{path}/out_imagenes_{dron}.png", bbox_inches="tight", pad_inches=0.1)
