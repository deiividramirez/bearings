                   ...
                   int drone_id)
{
   ...
   for (int32_t i = 0; i < (*states)[drone_id - 1].params.seguimiento.rows; i++)
   {
      ...
      else if (opc == 1)
      {
         ...
         suma1 += (-(*states)[drone_id - 1].actual.bearings.col(i) - (*states)[drone_id - 1].params.bearing.col(i));
      }
      else if (opc == 2)
      {
         // Control with bearing ortogonal projection
         cout << "[INFO] Control with bearing ortogonal projection" << endl;
         temp = (*states)[drone_id - 1].actual.bearings.col(i);
   ...
   (*states)[drone_id - 1].error = norm((*states)[drone_id - 1].actual.bearings + (*states)[drone_id - 1].params.bearing, NORM_L2);
   ...
   double lambda_Kp = (l0_Kp - linf_Kp) * exp(-(5 * (*states)[drone_id - 1].error) / (l0_Kp - linf_Kp)) + linf_Kp;
   (*states)[drone_id - 1].lambda_kp = lambda_Kp;

   double l0_Kv = (*states)[drone_id - 1].Kw_max, linf_Kv = (*states)[drone_id - 1].Kw;
   double lambda_Kv = (l0_Kv - linf_Kv) * exp(-(-0.4 * (*states)[drone_id - 1].error) / (l0_Kv - linf_Kv)) + linf_Kv;
   ...
   cout << "Actual bearing: " << (*states)[drone_id - 1].actual.bearings << endl;
